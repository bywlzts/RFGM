...
class model(nn.Module):
    def __init__(self, nf=64):
        ...
        self.AmpNet = RFGM(8)
        self.nf = nf
        self.mambablock = PatchMamba(input_channels=self.nf, num_blocks=(3, 3), num_mamba_layers=4)
        self.encoder_decoder = GradMamba(nf=nf)

    def forward(self, x):
        _, _, H, W = x.shape
        img_fourier_enhanced = self.AmpNet(x)
        x_center = img_fourier_enhanced
        ...
        out_mamba = self.mambablock(x_center)
        out_mamba = out_mamba[:, :, :H, :W]
        out_noise = out_mamba + out_noise

        return out_noise, img_fourier_enhanced